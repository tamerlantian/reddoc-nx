...
import {
  PUESTOS_COLUMNS,
  PUESTOS_FILTER_FIELDS,
  PUESTOS_FILTERS_STORAGE_KEY,
  PUESTOS_QUICK_SEARCH_FIELD,
  PUESTOS_PRIMARY_ACTION,
  PUESTOS_ROW_ACTIONS,
  PUESTOS_TRAILING_ACTIONS,
} from '../../puesto.constants';
...
@Component({
  ...
  styleUrl: './puestos-list.component.scss',
})
export class PuestosListComponent {
  // ── Colaboradores ─────────────────────────────────────────────────────────
  private readonly service = inject(PuestoService);
  ...
  // ── Estado ────────────────────────────────────────────────────────────────
  protected readonly items = signal<readonly Puesto[]>([]);
  protected readonly totalCount = signal(0);
  protected readonly isLoading = signal(false);
  protected readonly currentPage = signal(0);
  protected readonly pageSize = signal(25);
  protected readonly sort = signal<readonly SortSpec[]>([]);
  protected readonly selectedRows = signal<readonly Puesto[]>([]);
  protected readonly searchValue = signal('');
  protected readonly activeFilters = signal<readonly FilterCondition[]>(
    this.filterStorage.read(PUESTOS_FILTERS_STORAGE_KEY),
  );
  ...
  /**
   * Migas: módulo (Turno, navegable a su home) → entidad actual (Puestos).
   * El puesto es un master del módulo `turno`, por eso el segmento es fijo.
   */
  ...
  protected readonly columns = PUESTOS_COLUMNS;
  protected readonly filterFields = PUESTOS_FILTER_FIELDS;
  protected readonly rowActions = PUESTOS_ROW_ACTIONS;
  protected readonly primaryAction = PUESTOS_PRIMARY_ACTION;
  protected readonly trailingActions = PUESTOS_TRAILING_ACTIONS;
  ...
  protected onFiltersApply(filters: readonly FilterCondition[]): void {
    ...
    this.filterStorage.write(PUESTOS_FILTERS_STORAGE_KEY, filters);
    this.currentPage.set(0);
    this.loadList();
  }
  ...
  protected clearFilters(): void {
    ...
    this.filterStorage.clear(PUESTOS_FILTERS_STORAGE_KEY);
    this.currentPage.set(0);
    this.loadList();
  }

  protected onSelectionChange(rows: unknown[]): void {
    this.selectedRows.set(rows as Puesto[]);
  }
  ...
  protected onRowAction(event: RowActionInvokedEvent): void {
    ...
    switch (event.actionId) {
      case 'view':
        this.navigateToDetail(puesto.id);
        break;
      case 'edit':
        this.navigateToEdit(puesto.id);
        break;
      case 'delete':
        this.confirmRemove([puesto.id]);
        break;
    }
  }
  ...
  private loadList(): void {
    ...
    const search = quickSearchCondition(PUESTOS_QUICK_SEARCH_FIELD, this.searchValue());
   ...
   * Resulta en `/t/<slug>/puestos/<...path>`.
   */
