...
/** Cabecera legible de la remisión para la ficha (solo lo que trae `getById`). */
...
@Component({
  ...
  templateUrl: './remision-detail.component.html',
  styleUrl: './remision-detail.component.scss',
})
export class RemisionDetailComponent implements OnInit {
    ...
    forkJoin({
      ...
      .subscribe({
        ...
        error: () => {
          this.isLoading.set(false);
          this.notFound.set(true);
          const toasts = this.t().entities.remision.form.toasts;
