...
import { depreciacionLineaToView } from '../../depreciacion-linea.mapper';
import type { DepreciacionLineaRead, DepreciacionLineaView } from '../../depreciacion-linea.model';
import { depreciacionToFormValue } from '../../depreciacion.mapper';
import type { DepreciacionRead } from '../../depreciacion.model';

/** Cabecera legible de la depreciación para la ficha (solo lo que trae `getById`). */
...
/**
 * Ficha (detalle) de una **Depreciación** — solo lectura.
 *
 * Monta la misma tabla de líneas que el formulario pero sin acciones: acá no se
 * cargan activos ni se eliminan líneas, eso es trabajo de la edición. Carga
 * cabecera (`ENTITY_DATA_GATEWAY`) y líneas (`DocumentoDetalleService`) en
 * paralelo, igual que el form.
 */
@Component({
  selector: 'app-depreciacion-detail',
  standalone: true,
  imports: [
    ButtonModule,
    ConfirmDialogModule,
  ...
  templateUrl: './depreciacion-detail.component.html',
  styleUrl: './depreciacion-detail.component.scss',
})
export class DepreciacionDetailComponent implements OnInit {
  ...
  /** Líneas del documento, ya mapeadas a la forma del front para la tabla. */
    ...
    forkJoin({
      ...
      .subscribe({
        next: ({ cabecera, lineas }) => {
          const read = cabecera as DepreciacionRead;
