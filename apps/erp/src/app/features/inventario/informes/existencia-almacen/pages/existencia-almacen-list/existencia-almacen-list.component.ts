...
import {
  ...
  EXISTENCIA_ALMACEN_FILTER_FIELDS,
  EXISTENCIA_ALMACEN_FILTERS_STORAGE_KEY,
  EXISTENCIA_ALMACEN_TRAILING_ACTIONS,
} from '../../existencia-almacen.constants';

/**
 * Informe **Existencias por almacén** del módulo Inventario.
 *
 * Lista de solo lectura con grano ítem × almacén: paginación, filtros y
 * descarga de Excel. No tiene crear/editar/eliminar ni selección múltiple
 * (camino B recortado a un informe). Reusa los building blocks compartidos
 * (`<lib-data-toolbar>` + `<lib-data-table>`).
 */
@Component({
  selector: 'app-existencia-almacen-list',
  standalone: true,
  imports: [ListShellComponent, DataTableComponent, DataToolbarComponent, DataFilterModalComponent],
  templateUrl: './existencia-almacen-list.component.html',
  styleUrl: './existencia-almacen-list.component.scss',
})
export class ExistenciaAlmacenListComponent {
  // ── Colaboradores ─────────────────────────────────────────────────────────
  private readonly service = inject(ExistenciaAlmacenService);
  ...
  // ── Estado ────────────────────────────────────────────────────────────────
  protected readonly items = signal<readonly ExistenciaAlmacen[]>([]);
  ...
  protected readonly breadcrumbItems = computed<readonly BreadcrumbItem[]>(() => {
    ...
    return [
      ...
      { label: this.t().entities.existenciaAlmacen.name },
    ];
  });

  protected readonly columns = EXISTENCIA_ALMACEN_COLUMNS;
  protected readonly filterFields = EXISTENCIA_ALMACEN_FILTER_FIELDS;
  protected readonly trailingActions = EXISTENCIA_ALMACEN_TRAILING_ACTIONS;
  ...
  protected onFiltersApply(filters: readonly FilterCondition[]): void {
    ...
    this.filterStorage.write(EXISTENCIA_ALMACEN_FILTERS_STORAGE_KEY, filters);
  ...
  protected clearFilters(): void {
    ...
    this.filterStorage.clear(EXISTENCIA_ALMACEN_FILTERS_STORAGE_KEY);
    this.currentPage.set(0);
    this.loadList();
  }
