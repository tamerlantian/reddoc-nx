...
import { notaDebitoCompraToFormValue } from '../../nota-debito-compra.mapper';
import type { NotaDebitoCompraRead } from '../../nota-debito-compra.model';

/** Cabecera legible de la nota débito para la ficha (solo lo que trae `getById`). */
...
/**
 * Ficha (detalle) de una **Nota débito de compra** (familia comercial) — solo
 * lectura.
 *
 ...
 */
@Component({
  selector: 'app-nota-debito-compra-detail',
  standalone: true,
  imports: [
    ButtonModule,
    ConfirmDialogModule,
  ...
  templateUrl: './nota-debito-compra-detail.component.html',
  styleUrl: './nota-debito-compra-detail.component.scss',
})
export class NotaDebitoCompraDetailComponent implements OnInit {
    ...
    forkJoin({
      ...
      .subscribe({
        next: ({ cabecera, lineas }) => {
          const read = cabecera as NotaDebitoCompraRead;
          const nd = notaDebitoCompraToFormValue(read);
          this.cabecera.set({
