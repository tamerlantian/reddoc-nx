...
import { documentoSoporteToFormValue } from '../../documento-soporte.mapper';
import type { DocumentoSoporteRead } from '../../documento-soporte.model';

/** Cabecera legible del documento soporte para la ficha (solo lo que trae `getById`). */
...
/**
 * Ficha (detalle) de un **Documento soporte** (familia comercial) — solo lectura.
 *
 ...
 */
@Component({
  selector: 'app-documento-soporte-detail',
  standalone: true,
  imports: [
    ButtonModule,
    ConfirmDialogModule,
  ...
  templateUrl: './documento-soporte-detail.component.html',
  styleUrl: './documento-soporte-detail.component.scss',
})
export class DocumentoSoporteDetailComponent implements OnInit {
    ...
    forkJoin({
      ...
      .subscribe({
        next: ({ cabecera, lineas }) => {
          const read = cabecera as DocumentoSoporteRead;
