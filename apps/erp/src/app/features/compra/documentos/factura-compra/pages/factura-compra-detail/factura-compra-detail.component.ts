...
import { facturaCompraToFormValue } from '../../factura-compra.mapper';
...
/**
 * Ficha (detalle) de una **Factura de compra** (familia comercial) — solo lectura.
 *
 * Camino A del enfoque híbrido: la cabecera comercial es específica de cada
 * documento (de ahí que viva en `factura-compra/` y no en un `_shared`), pero la
 ...
 */
@Component({
  selector: 'app-factura-compra-detail',
  standalone: true,
  imports: [
    ButtonModule,
    ConfirmDialogModule,
    ...
    forkJoin({
      ...
      .subscribe({
        next: ({ cabecera, lineas }) => {
          const read = cabecera as FacturaCompraRead;
