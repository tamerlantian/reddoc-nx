...
import { egresoToFormValue } from '../../egreso.mapper';
import type { EgresoRead } from '../../egreso.model';

/** Cabecera legible del egreso para la ficha (solo lo que trae `getById`). */
...
/**
 * Ficha (detalle) de un **Egreso** (desembolso de tesorería) — solo lectura.
 *
 * Espeja `PagoDetailComponent` en sentido CxP: la cabecera del desembolso es
 * corta (proveedor, fecha, cuenta banco, comentario) y las líneas son asientos
 * —cuenta + naturaleza + valor, con contacto y centro de costo—, así que compone
 * la tabla contable read-only y su resumen (débitos, créditos y el neto que es
 * el desembolso). Carga cabecera (`ENTITY_DATA_GATEWAY`) y líneas
 * (`DocumentoDetalleService`) en paralelo, igual que el form. Desde aquí se
 * vuelve a la lista o se salta a editar.
 */
@Component({
  selector: 'app-egreso-detail',
  standalone: true,
  imports: [
    ButtonModule,
    ConfirmDialogModule,
...
export class EgresoDetailComponent implements OnInit {
  ...
  protected readonly resumen = computed<ResumenContable>(() =>
    calcularResumenContable(this.lines(), 'pagar'),
  );
    ...
    forkJoin({
      ...
      .subscribe({
        next: ({ cabecera, lineas }) => {
          const read = cabecera as EgresoRead;
          const pv = egresoToFormValue(read);
          this.cabecera.set({
