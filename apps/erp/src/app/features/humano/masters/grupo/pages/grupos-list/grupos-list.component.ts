...
export class GruposListComponent {
  ...
  protected readonly items = signal<readonly Grupo[]>([]);
  ...
  protected onFiltersApply(filters: readonly FilterCondition[]): void {
    ...
    this.filterStorage.write(GRUPOS_FILTERS_STORAGE_KEY, filters);
    this.currentPage.set(0);
    this.loadList();
  }
  ...
  protected clearFilters(): void {
    ...
    this.filterStorage.clear(GRUPOS_FILTERS_STORAGE_KEY);
    this.currentPage.set(0);
    this.loadList();
  }

  protected onSelectionChange(rows: unknown[]): void {
    this.selectedRows.set(rows as Grupo[]);
  }

  protected onRowAction(event: RowActionInvokedEvent): void {
    const grupo = event.row as Grupo;
    switch (event.actionId) {
      case 'view':
        this.navigateTo('detalle', grupo.id);
        break;
      case 'edit':
        this.navigateTo('editar', grupo.id);
        break;
      case 'delete':
        this.confirmRemove([grupo.id]);
        break;
    }
  }
  ...
  private loadList(): void {
    const search = quickSearchCondition(GRUPOS_QUICK_SEARCH_FIELD, this.searchValue());
  ...
  private navigateTo(...subPath: (string | number)[]): void {
    ...
    void this.router.navigate(['/t', slug, 'humano', 'grupos', ...subPath]);
  }
}
